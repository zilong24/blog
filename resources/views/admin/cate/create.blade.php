	...
	<div class="main-content">
		...
		<div id="page-wrapper">
			<div class="main-page">
				...
				<form id="form1" action="/admin/cate/store" method="post" enctype="multipart/form-data" >
				  {{csrf_field()}}
				  <input type="hidden" id="path" name="path" value="">
				  <div class="form-group">
				    <label for="cname" >分类名称</label>
				    <input type="text" class="form-control" id="cname" name="cname" placeholder="" value="{{old('cname')}}">
				  </div>
				  <div class="form-group">
				    ...
				    <select id="pid" name="pid">
						...
						<option value="{{$v->id}}" data-path="{{$v->path}}" @if($v->pid > 0) disabled @endif>{{$v->cname}}</option>
						@endforeach
				    </select>
				  </div>
